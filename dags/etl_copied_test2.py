...
@task()
def get_src_tables():
    hook = MsSqlHook(mssql_conn_id="SQL_Server_window_remote")
...
def load_src_data(tbl_dict: dict):
    conn = BaseHook.get_connection('MySQL_Server_Ubuntu')
...
with DAG(dag_id="product_etl_dag_test2",schedule_interval="0 9 * * *", start_date=datetime(2022, 3, 5),catchup=False,  tags=["product_model"]) as dag:
    ...
    #src_product_tbls >> load_dimProducts >> transform_src_product >> load_product_model
